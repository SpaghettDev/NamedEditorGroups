...
struct NIDSetupCountTriggerPopup : geode::Modify<NIDSetupCountTriggerPopup, SetupCountTriggerPopup>
{
	...
	{
		if (!SetupCountTriggerPopup::init(p0, p1)) return false;
		...
		STP->m_fields->m_id_inputs.insert({ ITEM_ID_PROPERTY, std::move(itemIDInputInfo) });
	...
	{
		SetupCountTriggerPopup::onItemIDArrow(sender);
	...
	{
		SetupCountTriggerPopup::onTargetIDArrow(sender);
	...
	{
		SetupCountTriggerPopup::textChanged(input);
