// https://github.com/EclipseMenu/EclipseMenu/blob/1b0ec40d8af1a14d4e573b2fabd6b02606a67423/src/modules/utils/VMTHooker.hpp#L66
// Copyright 2025 EclipseTeam
...
namespace ng::utils
{
	...
	private:
		// TODO: remove if geode::addresser::Addresser::cachedInstance is made public
		// or when i figure out how to construct a custom geode ctor type without
		// the dtor getting called 
		VMTHooker(TargetType* instance) {
	...
	public:
		static VMTHooker& get(TargetType* instance)
				requires(std::is_constructible_v<TargetType, geode::CutoffConstructorType, std::size_t>)
		{
			static VMTHooker hooker{ instance };
			return hooker;
		}
	...
	R traits::FunctionTraits<R(C::*)(Args...)>::trampoline(C* self, Args... args) {
		if constexpr (std::is_constructible_v<typename H::TargetType, geode::CutoffConstructorType, std::size_t>)
		{
			auto& hook = H::get(reinterpret_cast<typename H::TargetType*>(self));
			hook.m_callCount = 0;
